...
feature {NONE, ES_TEST} -- implementation
	...
	remove (a_node: NODE[G])
		...
		do
			-- To Be Done
			-- keep track of previous and next nodes, then assign the previous node's next attribute to the a_node's next, and next node's attribute
			-- to a_node's previous to eliminate a_node's attributes.
			l_pred := a_node.previous;
			l_succ := a_node.next;
			check attached l_pred as prev and then attached l_succ as next then
				prev.set_next (next)
				next.set_prev (prev)
			end
			count := count - 1
...
feature -- commands
	...
	add_last(e: detachable G)
		...
		do
			...
			-- To Be Done
			-- Must check that count increased by 1 and that last node has element e
			increment_count_al: count = old count + 1
			is_attatched: attached trailer.previous as last
	...
	remove_last
		...
		do
			...
			-- To Be Done
			count_decremented_rl: count = old count - 1
			last_removed: not node_exists ((old node(count)))
		end
	...
	add_before(n: NODE[G]; e: detachable G)
		...
		do
			...
			-- use add_between
			prev_n := n.previous;
			check attached prev_n as pn then
			    add_between(e, pn, n)
			end
		ensure
	...
	add_after(n: NODE[G]; e: detachable G)
		...
		do
			...
			-- use add_between
			next_n := n.next;
			check attached next_n as nn then
			    add_between(e, n, nn)
			end
		ensure
	...
	add (e: detachable G; i: INTEGER_64)
		...
		do
			...
			check attached l_pred as lp then
				add_between(e, lp, node(i))
			end
		ensure
	...
	remove_at (i: INTEGER)
		...
		do
			...
			-- To be Done
			count_decremented_ra: count = old count - 1
			removed_at: not node_exists ((old node(i)))
		end
